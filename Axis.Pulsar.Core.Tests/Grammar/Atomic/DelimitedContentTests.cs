...
using static Axis.Pulsar.Core.Grammar.Atomic.DelimitedContent;
...
namespace Axis.Pulsar.Core.Tests.Grammar.Atomic
{
    ...
    public class DelimitedContentTests
    {
        ...
        public void TryRecognizeDelimiter_Tests()
        {
            ...
            var recognized = DelimitedContent.TryRecognizeDelimiter("", delimiter, out var tokens);
            Assert.IsFalse(recognized);
            Assert.IsTrue(tokens.IsEmpty);

            recognized = DelimitedContent.TryRecognizeDelimiter("adg", delimiter, out tokens);
            Assert.IsFalse(recognized);
            Assert.IsTrue(tokens.IsEmpty);

            recognized = DelimitedContent.TryRecognizeDelimiter("'", delimiter, out tokens);
        ...
        public void TryRecognizeContent_Tests()
        {
            ...
            var recognized = DelimitedContent.TryRecognizeContent("", constraint, true, out var tokens);
            Assert.IsTrue(recognized);
            Assert.IsTrue(tokens.IsEmpty);

            recognized = DelimitedContent.TryRecognizeContent("", constraint, false, out tokens);
            Assert.IsFalse(recognized);
            Assert.IsTrue(tokens.IsEmpty);

            recognized = DelimitedContent.TryRecognizeContent("abcd\uf2fc", constraint, false, out tokens);
            Assert.IsTrue(recognized);
    ...
    public class LegalDiscretePatternsTests
    {
        ...
        public void Constructor_Tests()
        {
            ...
            Assert.ThrowsException<ArgumentNullException>(() => new LegalDiscretePatterns(default(IPattern[])!));
    ...
    public class IllegalDiscretePatternTests
    {
        ...
        public void Constructor_Tests()
        {
            ...
            Assert.ThrowsException<ArgumentNullException>(() => new IllegalDiscretePatterns(new DelimiterInfo("^", "\\^"), default(IPattern[])!));
