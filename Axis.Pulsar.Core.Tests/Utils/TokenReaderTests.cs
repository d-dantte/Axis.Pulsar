...
namespace Axis.Pulsar.Core.Tests.Utils
{
    ...
    public partial class TokenReaderTests
    {
        ...
        public void TryPeekOrReadTokens_Tests()
        {
            ...
            Assert.AreEqual(1, tokens.SourceSegment.Length);
            Assert.AreEqual('t', tokens[0]);
            Assert.AreEqual(0, reader.Position);

            success = reader.TryGetToken(out var tokens2);
            Assert.IsTrue(success);
            Assert.AreEqual(1, tokens2.SourceSegment.Length);
            ...
            Assert.IsTrue(success);
            Assert.AreEqual(10, tokens.SourceSegment.Length);
            Assert.IsTrue(tokens.Equals("he quick b"));
            Assert.AreEqual(1, reader.Position);

            success = reader.TryGetTokens(10, out tokens2);
            Assert.IsTrue(success);
            Assert.AreEqual(10, tokens2.SourceSegment.Length);
            ...
            Assert.IsTrue(success);
            Assert.AreEqual(41, tokens.SourceSegment.Length);
            ...
            Assert.IsTrue(success);
            Assert.AreEqual(41, tokens2.SourceSegment.Length);
